...
const EditPost = ({post, categories}) => {
    ...
    const [formData, setFormData] = useState({
        ...
        show: post.show,
        initialPhoto: post.postImg
      }) 

    const [editorState, setEditorState] = useState(EditorState.createEmpty())  
    
    const { title, excerpt, body, category, postImg, show, initialPhoto } = formData
    ...
    const handleSubmit = async e => {
        ...
        if(!title || !excerpt || !body || category.length === 0)
        return dispatch({type: 'NOTIFY', payload: {error: 'Please add all the fields.'}})

        dispatch({ type: 'NOTIFY', payload: {loading: true} })

        if(postImg && initialPhoto !== postImg) media = await imageUpload([postImg])

        const res = await putData(`post/${post._id}`, { 
            title, 
            body, 
            category,
            excerpt,
            show,
            postImg: initialPhoto !== postImg ? media[0].url : postImg
            }, auth.token)
            ...
            <form onSubmit={handleSubmit} className="sm:px-6 pt-6 pb-8 mb-4 text-sm font-semibold">
                ...
                <div className="mb-6">
                    <div className="h-10 w-10 mb-4">
                        <img src={postImg} className="w-full h-full object-cover" />
                    </div>
                    <label className="block mb-2">
                    Feature Image
                    </label>
                ...
                <div className="mb-6 flex space-x-4 items-center">
                    <div className="flex space-x-2 items-center">
                        <input type="radio" name="show" value={false} onChange={handleShow} checked={show === false && true} className="checked:bg-gray-900 checked:border-transparent scale-125" />
                        <span>Draft</span>
                    </div>
                    <div className="flex space-x-2 items-center">
                        <input type="radio" name="show" value={true} onChange={handleShow} checked={show && true} className="checked:bg-gray-900 checked:border-transparent scale-125" />
                        <span>Publish</span>
                    </div>
                </div>
                <div>
...
export async function getServerSideProps({params: {id}}) {
    ...
    const catRes = await getData('category')
    if (res.err) {
      return {
        notFound: true,
      }
